...
export function ImportModal({ title, importUrl, previewColumns, onClose, onImported }) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const result = await confirmImport(selectedRows);
  ...
  return (
    <Modal isOpen onClose={onClose} title={title} size="lg">
      <div className="flex flex-col gap-4">
        {!preview && (
          <label className="flex cursor-pointer flex-col items-center gap-2 rounded-lg border-2 border-dashed border-gray-300 p-6 text-center text-sm text-gray-500 hover:border-red-400 hover:text-red-600">
            <Upload size={24} />
